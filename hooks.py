...
# my modification
def my_add_hook_to_module(module: nn.Module, hook: ModelHook, append: bool = False):
    """

    Returns:
        torch.nn.module with its origin forward, with new attr 'next hook' link to the next module
...
def my_attach_execution_device_hook(
        module: torch.nn.Module,
        execution_device: Union[int, str, torch.device],
):
    if not hasattr(module, "_hf_hook") and len(module.state_dict()) > 0:
...
def attach_align_device_hook(
    ...
    for child_name, child in module.named_children():
        ...
        hooked_module = attach_align_device_hook(
            child,
            execution_device=execution_device,
            offload=offload,
...
def attach_align_device_hook_on_blocks(
    ...
    if module_name in execution_device and module_name in offload and not offload[module_name]:
        ...
        # Todo: module_name, 部分模型 = add_hook_to_module，通过返回值再判断是否获取
...
def my_attach_align_device_hook_on_blocks(
        module: nn.Module,
        module_dict: Dict,
        execution_device: Optional[Union[torch.device, Dict[str, torch.device]]] = None,
        offload: Union[bool, Dict[str, bool]] = False,
    ...
    if not isinstance(execution_device, Mapping) and not isinstance(offload, dict):
        if not offload:
            hook = AlignDevicesHook(execution_device=execution_device, io_same_device=True, place_submodules=True)
            module = add_hook_to_module(module, hook)
        else:
            module = attach_align_device_hook(
                module,
                execution_device=execution_device,
                offload=True,
                weights_map=weights_map,
    ...
    if module_name in execution_device and module_name in offload and not offload[module_name]:
        ...
        module_without_offload = add_hook_to_module(module, hook)
        module_dict[module_name] = module_without_offload
